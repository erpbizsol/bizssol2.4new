...
@Component({
  ...
  standalone: true,
  providers: [CountryService,FormControlName,StateService],
  imports: [HttpClientModule,MatTableModule,MatPaginatorModule,MatSortModule, ReactiveFormsModule,MatIconModule, CommonModule,ButtonDirective, ModalComponent, ModalHeaderComponent, ModalTitleDirective, ThemeDirective, ButtonCloseDirective, ModalBodyComponent, ModalFooterComponent,MatInputModule,MatSelectModule],
  templateUrl:'./state-table.component.html',
  styleUrl: './state-table.component.scss',
})
export class StateTableComponent {

  displayedColumns: string[] = ['sNo', 'statename','stateshortname','statecode','countryname', 'action'];
  ...
  constructor(private _countryService:CountryService, private _stateService:StateService ,public dialog:MatDialog){}

  ngOnInit(){
    this.getCountryList();
    this.getStateData('All');
  ...
  getStateData(country:string) {
    ...
    this._stateService.getStatesList(this.selectedcountry).subscribe({
      ...
      next: (res: any) => {
        ...
        this.statelist=res;
        console.log(this.statelist,"Hial");
      },
      error: (err: any) => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.confirmed) {
        ...
        this._countryService.deleteCountry(code, reason).subscribe((res) => {
          console.log(`${code} has been deleted`);
          const responseObj = JSON.parse(JSON.stringify(res));
          this.getStateData(this.selectedcountry);
          alert(responseObj.Msg);
        });
      }
    });
